...
const unsigned long periodSpeakerPulse = 1;
const unsigned long periodIncrementFreq = 50;
const unsigned long periodDecrementFreq = 50;
const unsigned long periodDeterminePer = 50;

/* shared variables */
unsigned char threeLEDs;
unsigned char blinkingLED;
unsigned char speakerPulse;
unsigned char soundPeriod;
unsigned char up;
unsigned char down;
...
int DeterminePeriodSM(int state) {
	...
	switch(state) { // actions
		...
		case inf:
			...
			else if (down && !up && (soundPeriod < 10)){
				soundPeriod ++;
			}			
			break;
		default:
...
int main(void) {
	...
	tasks[i].state = IF_start;
	tasks[i].period = periodIncrementFreq;
	tasks[i].elapsedTime = 0;
	tasks[i].TickFct = &IncrementFreqSM;
	i++;

	tasks[i].state = DF_start;
	tasks[i].period = periodDecrementFreq;
	tasks[i].elapsedTime = 0;
	tasks[i].TickFct = &DecrementFreqSM;
	i++;

	tasks[i].state = DP_start;
	tasks[i].period = periodDeterminePer;
	tasks[i].elapsedTime = 0;
	tasks[i].TickFct = &DeterminePeriodSM;
	i++;
